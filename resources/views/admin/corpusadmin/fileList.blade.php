...
<div class="container-fluid archives">
    @if(!empty($fileData["corpusData"]['projects']))
        <div class="row">
            <div class="col-md-12">
                <div class="panel panel-default">
                    <form id="fileform">
                        <table class="table table-bordered table-hover vmiddle">
                            ...
                            <tbody>
                            @if($fileData["corpusData"]['projects'])
                                @foreach($fileData["corpusData"]['projects'] as $project)
                                    @if($project["type"] == "dir")
                                        <tr>
                                            <td>
                                            </td>
                                            ...
                                            <td class="text-center">
                                                ...
                                                @if($project["tracked"] == "false" && $project["foldercount"] > 0 && $fileData["corpusData"]["pathcount"] == 3)
                                    ...
                                    @else
                                        <tr id="cell_{{$loop->index}}">
                                            ...
                                            @endif
                                            <td class="text-center">
                                                @if($project["tracked"] == "false" && $fileData["corpusData"]["pathcount"] == 4)
                                                    <a href="{{route('gitRepo.addFile.route', array('path' => $project['path'],'corpus' => $corpus->id)) }}"><span class="btn btn-sm btn-danger fa fa-code-fork"></span></a>
                                                @endif
    ...
    @endif
        @if($fileData["corpusDataFolder"] == "CORPUS-DATA")
            <span class="btn btn-sm btn-primary fa fa-file-code-o" data-toggle="modal" data-target="#myModal"> Add new format</span>
        @endif
        <span class="pull-right"><a href="{{route('gitRepo.uploadFiles.get',array('dirname' => $fileData["corpusData"]["path"])) }}" ><i class="fa fa-upload fa-3x" aria-hidden="true"></i></a></span>
